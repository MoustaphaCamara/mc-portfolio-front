...
const Skills = () => {
  ...
  const [filter, setFilter] = useState<Filter.Skills>(Filter.Skills.All);
    ...
    <div id={NavList.Skills}>
      ...
      <div className="app__skills-filter">
        {queryList.map((item) => (
          <Button
            key={item}
            content={
              item === Filter.Skills.All
                ? t('filters.all')
                : t(`filters.skills.${item}`)
            }
            status={filter}
            setStatus={setFilter}
            value={item}
      ...
      <div className="app__skills-complementary">
        <div className="app__skills-complementary-items">
          <p className="p-text">{t('skills.additionalFirstLine')}</p>
        </div>
        <div className="app__skills-complementary-desktop">
          <p className="p-text">{t('skills.additionalSecondLine')}</p>
        </div>
      </div>
    </div>
  );
